...
def _worker(index: int, config: dict[str, object]) -> None:
    ...
    if resume_path:
        ...
        start_step = validate_full_resume(
            payload, minimum_step=int(config["expected_resume_step"])
        )
        checkpoint_world_size = payload.get("execution", {}).get("world_size")
        if checkpoint_world_size is not None and int(checkpoint_world_size) != world_size:
            raise RuntimeError(
                f"checkpoint used world_size={checkpoint_world_size}, current runtime uses "
                f"{world_size}; refusing topology drift"
            )
        if rank == 0:
            print(
                f"[TPU resume] step={start_step:,} "
