...
from anra_paths import DRIVE_DIR, DRIVE_V2_CHECKPOINTS, ROOT, TRAINING_DATA_DIR, ensure_dirs, inject_all_paths
from anra_brain import CausalTransformerV2
from training.eval_v2 import run_compact_eval
from training.v2_config import MODEL_LINE, V2_MODEL, V2_REPORT_FILES, V2_TRAINING
...
def resolve_dataset_path(explicit: str | None) -> Path:
    ...
    if _valid_text_dataset(_DRIVE_DATASET):
        ...
        print(f"[V2 Trainer] dataset restored from Drive: {_CANONICAL_DATASET}", flush=True)
        return _CANONICAL_DATASET
    raise FileNotFoundError(
...
def run_cmd(cmd: list[str], *, cwd: Path | None = None) -> int:
    print("\n[V2 Trainer] Running:", " ".join(cmd), flush=True)
...
def _run_eval_only() -> dict[str, object]:
    ...
    summary = run_compact_eval(model, tokenizer, device=device, output=True)
    return summary


def main() -> None:
    ap = argparse.ArgumentParser(description="An-Ra V2 training dispatcher")
    ap.add_argument("--mode", default="session", choices=["session", "milestone", "eval", "status"])
    ...
    if args.mode == "status":
        print_system_health()
        print(f"[V2 Trainer] dataset={resolve_dataset_path(args.data_path)}")
        print(f"[V2 Trainer] brain_ckpt={canonical_v2_checkpoint('brain')}")
        print(f"[V2 Trainer] identity_ckpt={canonical_v2_checkpoint('identity')}")
        print(f"[V2 Trainer] ouroboros_ckpt={canonical_v2_checkpoint('ouroboros')}")
        print(f"[V2 Trainer] tokenizer={ROOT / 'tokenizer' / 'tokenizer_v2.json'}")
        print(f"[V2 Trainer] milestone={_milestone_due()}")
        return

    if args.mode == "eval":
        summary = _run_eval_only()
        print(json.dumps(summary, indent=2))
        return

    dataset = resolve_dataset_path(args.data_path)
    print(f"[V2 Trainer] dataset={dataset}", flush=True)
    print_system_health()
