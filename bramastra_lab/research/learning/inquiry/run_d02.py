    ...
    try:
      for seed in cfg["seeds"]:
        ...
        for name, data in (("one_step", paired.one_step), ("depth_two", paired.depth_two)):
            ...
            if time.perf_counter() >= deadline:
                raise RunTimeout(f"deadline reached before evaluation seed {seed} arm {name}")
            ...
            if time.perf_counter() >= deadline:
                raise RunTimeout(f"deadline reached after evaluation seed {seed} arm {name}")
        ...
        if time.perf_counter() >= deadline:
            raise RunTimeout(f"deadline reached during reporting seed {seed}")
        ...
        if manifest["status"] == "RUNNING":
            if manifest["elapsed_seconds"] > deadline_seconds:
                manifest["status"] = "TIMEOUT"
                manifest["failure"] = {"type": "RunTimeout", "message": "deadline exceeded before completion"}
            else:
