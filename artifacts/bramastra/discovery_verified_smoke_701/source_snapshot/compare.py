...
def _metric_record(parent: list[dict], child: list[dict], policy: str,
    ...
    for values in worlds.values():
        ...
        world_values.append((old_acc, new_acc, new_acc - old_acc))
    nrows = len(before)
    parent_accuracy = sum(r["correct"] for r in before) / nrows
    child_accuracy = sum(r["correct"] for r in after) / nrows
    parent_brier = sum((r["probability"] - r["label"]) ** 2 for r in before) / nrows
    child_brier = sum((r["probability"] - r["label"]) ** 2 for r in after) / nrows
