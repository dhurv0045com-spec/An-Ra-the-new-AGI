...
def build_demonstrations(worlds: list[RuleWorld], *, episodes: int, budget: int,
    ...
    tensors = [torch.from_numpy(np.asarray(x)) for x in (obs, lengths, targets, labels, masks, all_gains)]
    return Demonstrations(tensors[0].float(), tensors[1].long(), tensors[2].float(),
                          tensors[3].float(), tensors[4].bool(), tensors[5].float(),
                          {"world_ids_sha256": identity, "worlds": len(worlds), "episodes": episodes,
