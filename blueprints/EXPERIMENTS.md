...
A paired initialization canary now attacks the provisional `1/sqrt(2L)` residual-output scaling directly. Five CUDA seeds and three CPU seeds compare identical `normal(0,0.02)` draws with only attention-output and FFN-down matrices rescaled. CUDA scaled/unscaled end-to-end residual-growth ratios are 0.122/0.156/0.230 for deep/middle/wide, with gradient-spread ratios 0.604/0.650/0.731; CPU reproduces both directions. Classification is `SUPPORTED_LOCAL_SIGNAL_PROPAGATION`, with exact counts and finite/nonzero gradients throughout. This promotes residual-output scaling from convention to a local implementation prior—not to a learning or cognition result. The scaled CUDA stacks still grow 3.52×/4.22×/5.38× from embedding to last block, so target-TPU constructor replication and bounded real-update E4 checks remain mandatory.
