...
| Position | RoPE, base 10,000, native 4,096 only | LOCAL CONFORMANCE EVIDENCE / EXPERIMENT REQUIRED | Native implementation passes an independent oracle; base choice, extrapolation, and TPU/XLA remain open |
| Norm | pre-RMSNorm, epsilon 1e-5 | EVIDENCE-BACKED | Stable and computationally simple |
| QK norm | enabled candidate with affine Q/K scales initialized to 1 | EVIDENCE-BACKED scale control / EXPERIMENT REQUIRED cognition | Paired CPU/CUDA probes prove projection-scale invariance; discrimination and learning effects remain unknown |
...
At 5B tokens, 131,072 tokens/update means exactly 38,146 full updates plus one 127,488-token final partial update: 38,147 total, with no padding-token accounting and no 3,584-token overshoot. A 262,144 center would yield about 19,074 updates under the same exact-budget policy. The smaller center is preferred until E4 shows that the larger batch's throughput compensates for lower update resolution. Do not rewarm LR merely because a data pack or session changes. A continuation resumes the global token-indexed schedule and optimizer state. Batch changes require an explicit migration experiment and receipt.
...
Status: the framework-neutral transaction canary binds model/optimizer/scheduler/RNG/cursor/ledger/state inventories, enforces a single-writer parent fence, verifies every component hash/size, survives crashes before/after publication, rejects missing/corrupt components, and proves uninterrupted versus clean-copy resume over a deliberately non-divisible 10-token budget (`4+4+2`). The new `artifacts/v5/local_p35_checkpoint_canary.json` joins that transaction to the exact middle-P35 model and AdamW optimizer: two synthetic updates publish a content-addressed checkpoint, restore from a clean local copy, and reproduce the next update with zero parameter and optimizer-state error. The cursor canary separately proves packed-stream continuation. **EVIDENCE-BACKED local components and transaction semantics; NOT YET distributed, remote-durable, or TPU checkpoint evidence.** Cadence remains **STRONG INFERENCE** pending target serialization/storage measurement.
The separate `artifacts/v5/local_durability_canary.json` now exercises immutable content-addressed upload, idempotent same-byte publication, corruption/missing-object rejection, redownload hash equality, and a typed durability receipt. This is a local CAS simulation only; it does not promote cloud durability.
`v5_training.runner` adds the lifecycle fence around those primitives: an in-flight update is never a committed update, completion requires a committed target, and failure preserves the last durable parent for explicit recovery. Its state is canonically serializable and fail-closed. **EVIDENCE-BACKED local lifecycle contract; distributed runner supervision remains open.**
`v5_training.distributed` now defines the rank-sharded boundary: every rank binds its RNG, optimizer shard, cursor, data-shard identity, token contribution, and collective barrier, while the coordinator requires a complete unique rank set and exact global-token reconciliation. **EVIDENCE-BACKED schema/tests; target collectives and storage remain open.**
`v5_training.target_preflight` is the executable target gate: on TPU/XLA it must verify device identity, declared world size/ordinal, BF16 matmul finiteness, all-reduce, and device-RNG progression; on a CPU host it returns `BLOCKED_TORCH_XLA` with explicit missing dependencies. This is runtime plumbing evidence only, not training or throughput evidence.
...
**Development status in v0.4:** an executable evaluation-only generator emits 368 deterministic cases and 112 mechanically checked causal pairs at the certification setting. State cases randomize serialization independently of semantic time, place every cutoff between/after events, balance eight competing target values, and cover latest, intermediate, rollback, and precedence over interleaved variables. Rule induction covers eight latent operand structures in development, with disjoint structures in sealed/fresh. Training generators use a separate template namespace. Candidate order, context position, answer formats, state-query axes, rule structures, and difficulty axes are explicit audited dimensions; hidden truth is excluded from model views; development/sealed/fresh graph/template/domain/rule-structure namespaces are mechanically disjoint. Representation, selection, query-lift, realization, sensitivity, invariance, assistance, and intervention-dependence metrics are separate APIs.

The committed development receipt is `artifacts/e0/development_certificate.json`, suite SHA-256 `f7a1b6af1a2ac6dc381499a87592e0f8329ea47d0eee0dede112d6ea090b8b56`. It reports random/first/last/lexical/position/bag-of-words, broken-state, fixed-rule, direct-retrieval, and full-oracle controls. The state and rule shortcut audits pool eight independent seeds. `latest_fact`/`nearest_position` use an analytic random-serialization null; all other state controls use casewise candidate chance; every named control must stay within its null + 10 percentage points. The repair reduced bag-of-words from 81.77% to 4.17% and lexical overlap from 71.09% to 10.16%, versus a 10.37% casewise null. An independent surface parser/solver agrees on every case, including a 20-seed generator property sweep. Uniform-candidate chance, Wilson intervals, difficulty curves, pair-effect counts, and approximate sample-size planning are explicit. **This certifies development infrastructure invariants, not benchmark difficulty and not model cognition.**

The scorer boundary is separately executable in `artifacts/e0/scoring_adapter_certificate.json`. It enforces suffix-only candidate traces, hash-bound adapter identity, sum/token/byte aggregation, complete candidate rotation, and oracle/broken/random controls. The deterministic null exposed that summed likelihood selects the fewest-token candidate 65.625% of the time and byte normalization selects the constructed first-token/token-density pattern 84.375%. The exact random-weight middle-P35 audit then loaded the real local 16k/24k/32k tokenizers and compared 486 CPU/CUDA suffix scores: zero predictions differed, maximum absolute score error was 0.0028076, and relative RMS error was 1.286e-7. This certifies local implementation parity. It also rejects all three naive policies: sum selected a fewest-token candidate 100%, byte normalization 83.33%, and token normalization 50%--66.67%. Status is **LOCAL DEVICE PARITY PASS / POLICY OPEN / TPU PENDING**; `production_scoring_mode` remains unset. **EVIDENCE-BACKED local parity and null bias; EXPERIMENT REQUIRED for a defensible scoring policy.**

`artifacts/e2/scoring_policy_preregistration.json` froze the powered experiment before results: 256 independent triplets, five development P35 seeds, three tokenizers, crossed surface/hidden roles, two four-context neutral panels, exact token-length/suffix matching, DC-PMI primary, contextual calibration secondary, and naive modes retained only as falsification controls. An audit first invalidated fixture v1 because hidden role was determined by surface family; schema 2 repaired and gated every 6×3 contingency cell. All 15 CUDA development shards then ran for 0.07574 GPU-hours. Both calibrated policies selected the fewest-token role 100% for every tokenizer; hidden-label accuracy stayed at chance, panel agreement was 0.977–0.994, and decoy stability was 1.000. Therefore both policies fail the primary equivalence/per-seed gates and are rejected. CPU parity and fresh execution were stopped because they cannot rescue a failed development gate. `production_scoring_mode` remains null. **EVIDENCE-BACKED NEGATIVE RESULT / SCORER REDESIGN REQUIRED.**
...
The 64-per-family perturbation sweep adds numbers, identifiers, nonces, Unicode, spacing, and formal variants. Every candidate remains byte-exact with zero unknowns; overall tokens/byte are 0.63834/0.60568/0.59293 for 16k/24k/32k, with 24k intermediate on every family. This strengthens local surface coverage only and does not select a tokenizer or replace external custody.
...
A fifth paired probe compares exact P35 FP32 and BF16 stacks with identical scaled-residual weights and inputs. Three CUDA and three CPU short-context seeds pass every preregistered limit across all shapes. A separate three-seed CUDA replication at native 2,048 context also passes: worst logit cosine 0.999917, logit RMS error 1.289%, sampled-gradient cosine 0.999631, and gradient RMS error 2.717%. Across all receipts, worst loss relative error is 0.000118. **EVIDENCE-BACKED local initialization/backward parity through P35 native context:** BF16 is acceptable for bounded local canaries when logits/loss reduce in FP32. **EXPERIMENT REQUIRED:** 4k V5-A, target TPU/XLA parity, real optimizer updates, optimizer-state precision, long-duration drift, and adversarial/data-tail numerics. This does not authorize the main run.

A sixth canary checks the extracted P35 RoPE at native 4,096 against an independent float64 oracle. Fresh seeds 36101–36105 on CUDA and 36101–36103 on CPU pass for FP32 and BF16, including norm preservation and relative-shift invariance. Worst CUDA FP32 reference RMS is `1.404e-5` and relative-shift RMS `2.546e-5`; worst BF16 values are `0.002946` and `0.005451`. An earlier calibration receipt failed the original float32 round-off limits and is retained as negative evidence; analytically revised limits were then validated on fresh seeds. **EVIDENCE-BACKED local implementation conformance:** native 4k RoPE geometry is correct. **EXPERIMENT REQUIRED:** base selection, extrapolation, TPU/XLA, and learned-quality effects.

A seventh canary runs three deterministic AdamW updates and a save/load continuation on the exact P35 middle stack. CUDA sequence 32 and CPU sequence 16 both pass live-parameter ownership, parameter-hash change, nonzero moments, exact optimizer step advancement, and exact within-dtype equality of resumed parameters and optimizer states. BF16 is implemented with FP32 master parameters under BF16 autocast, yielding FP32 optimizer moments. The native-BF16-parameter control fails that state-dtype gate and is retained as negative evidence. The same gate now accepts fresh deep-narrow and wide-shallow arms on both devices; the harness releases the pre-save model before constructing the resumed copy to avoid host-RAM false failures. **EVIDENCE-BACKED wiring/resume continuity:** the corrected local path performs real updates. **EXPERIMENT REQUIRED:** long-run, distributed, and TPU/XLA optimizer behavior.
A fresh-seed 10-update replication on both devices remains finite and preserves exact optimizer-state resume, but remains only a bounded stability result.
The 1K-context extension passes calibrated tolerance-based resume for parameters and optimizer state on CUDA and CPU. A strict CUDA control can pass on one deterministic runtime or fail from reduction-order drift, so the production contract records strict and tolerance outcomes separately and gates on the registered tolerance rather than assuming cross-backend bitwise determinism.
The companion cursor canary consumes 423 tokens across shuffled content-addressed sequence boundaries, round-trips the cursor through JSON, reproduces the uninterrupted tail exactly on CPU and CUDA, and rejects manifest/offset tampering. This is local single-process continuity evidence; distributed sampler partitioning, real pack I/O, and durable remote restore remain experiment-required.
