...
- Exact-head Linux CI exposed one remaining raw-byte binding for the candidate-spec JSON; it now uses the same newline-invariant text hash. Checkpoint/evidence payload integrity remains byte-exact. Integration tests cover both LF and CRLF spec copies.
