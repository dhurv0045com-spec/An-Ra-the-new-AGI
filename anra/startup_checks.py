...
def assert_flash_sdp_ready(entrypoint: str) -> None:
    ...
    if not torch.cuda.is_available():
        warnings.warn(
            f"[{entrypoint}] Flash SDP unavailable: CUDA is unavailable. "
            "Training will continue without "
    ...
    if not torch.backends.cuda.is_flash_attention_available():
        warnings.warn(
            f"[{entrypoint}] Flash SDP unavailable: "
            "torch.backends.cuda.is_flash_attention_available() is False. "
    ...
    flash_sdp_enabled = getattr(torch.backends.cuda, "flash_sdp_enabled", lambda: False)
