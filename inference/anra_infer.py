...
def load_model_and_tokenizer(
    ...
    tokenizer_path: str,
) -> tuple[CausalTransformer, object, str]:
    ...
    print(
        f"Model loaded from {model_checkpoint_path}. Parameters: "
...
def generate(
    ...
    if not encoded_prompt:
        ...
        print(
            "Warning: Prompt contains no known characters. Starting generation from empty sequence."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt", type=str, required=True, help="The initial text prompt for generation."
    )
    parser.add_argument(
        "--max_new_tokens", type=int, default=100, help="Maximum number of new tokens to generate."
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    model, tokenizer, device = load_model_and_tokenizer(
        abs_model_checkpoint_path, abs_tokenizer_file_path
    )
