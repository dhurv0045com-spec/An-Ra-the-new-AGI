...
def speed_benchmark(
    ...
    for bs in batch_sizes:
        for sl in seq_lens:
            ...
            print(f"  bs={bs:>3}  seq={sl:>4}  {tps:>10,.0f} tok/s  {lat:>7.2f} ms/fwd")
...
def score_generation(prompt: str, output: str, tokenizer: object) -> GenerationQuality:
    """
...
def run_eval_suite(
    ...
    tokenizer: object,
    test_corpus: str,
    inference_pipeline: object | None = None,
    ...
    speed_results = speed_benchmark(
        model, vocab_size=vocab, batch_sizes=[1, 4], seq_lens=[32, 128], n_trials=5, device=device
    )
    ...
    print(
        f"      Params: {mem.n_params:,}  ({mem.param_mb:.1f} MB params, {mem.peak_mb:.1f} MB peak)"
    )
    ...
    if inference_pipeline is not None:
        ...
        for prompt in test_prompts:
            out = inference_pipeline.generate(
                prompt, max_new_tokens=60, strategy="top_p", temperature=0.8, top_p=0.95
            )
            q = score_generation(prompt, out, tokenizer)
            quality_rows.append(asdict(q))
            print(
                f"      TTR={q.type_token_ratio:.3f}  "
...
if __name__ == "__main__":
    ...
    sr = speed_benchmark(
        model, vocab_size=64, batch_sizes=[1], seq_lens=[32], n_trials=5, device="cpu"
    )
    ...
    results = run_eval_suite(
        model, tok, corpus, output_dir="/tmp/eval_test", label="smoke", device="cpu"
    )
