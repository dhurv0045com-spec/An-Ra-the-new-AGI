"""FORMATION-MUX-001 Science-S3 worker: one arm, one process, one GPU."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        receipt = {
            "schema": "anra.formation-mux-worker/v3",
            "experiment": args.experiment,
            "arm": args.arm,
            "seed_bundle": args.seed_bundle,
        ...
        payload = {
            "schema": "anra.formation-mux-worker-failure/v3",
            "class": "GLOBAL_INTEGRITY_FAILURE" if code == EXIT_GLOBAL else "ARM_LOCAL_ENGINEERING_FAILURE",
            "exception": type(exc).__name__,
            "message": message,
            "traceback": traceback.format_exc(),
        }
        print(("GLOBAL_FAILURE " if code == EXIT_GLOBAL else "ARM_LOCAL_FAILURE ") + json.dumps(payload), flush=True)
        return code
    except Exception as exc:
        print(
            "GLOBAL_FAILURE " + json.dumps(
