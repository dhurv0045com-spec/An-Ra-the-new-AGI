"""MIXED-CAUSAL-v8: second confirming draw for v7 (replication hardening).
...
SEED_MC8 = 20270214
PREFIXES_MC8 = ("CGV", "HKN", "LQZ", "NWD", "RJS", "XMT")
ENTITIES_MC8 = ("balustrade-rail", "corbel-table", "dripstone-edge",
                "extrados-arch", "finial-knob", "reredos-panel")
COLORS_MC8 = ("sage-green", "burnt-sienna", "lapis", "chalk-white")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC8)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC8)
        ent = rng.choice(ENTITIES_MC8)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC8, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC8, n)]
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "schema": "anra-mixed-causal/v8-confirmation",
