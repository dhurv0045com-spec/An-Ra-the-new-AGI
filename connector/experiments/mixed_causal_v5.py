"""MIXED-CAUSAL-v5: fresh promotion fixture for self-model v5.

Generated ONLY after v5 freeze (commit 438b2ca). Fresh seed, entities,
...
SEED_MC5 = 20261117
PREFIXES_MC5 = ("DGX", "KRV", "PZB", "WJM", "ZTF")
ENTITIES_MC5 = ("flying-buttress", "hood-mould", "keystone-relief",
                "mullion-shaft", "ogee-arch")
COLORS_MC5 = ("ochre", "crimson", "cobalt", "ivory")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC5)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC5)
        ent = rng.choice(ENTITIES_MC5)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC5, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC5, n)]
