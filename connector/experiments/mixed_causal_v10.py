"""MIXED-CAUSAL-v10: promotion fixture for the 4-action v8 policy.
...
SEED_MC10 = 20270418
PREFIXES_MC10 = ("EZK", "HQD", "JYN", "LWA", "TRC", "UFG")
ENTITIES_MC10 = ("boss-moulding", "crenellation-cap", "diaper-work",
                "flying-arch", "gable-cross", "niched-panel")
COLORS_MC10 = ("weld-green", "tyrian", "bone", "charcoal")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC10)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC10)
        ent = rng.choice(ENTITIES_MC10)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC10, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC10, n)]
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "schema": "anra-mixed-causal/v10-promotion",
