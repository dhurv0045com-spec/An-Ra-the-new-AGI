"""MIXED-CAUSAL-v9: checkpoint-transfer probe for the v7 self-model.
...
SEED_MC9 = 20270321
PREFIXES_MC9 = ("DJP", "GXT", "KQF", "MWL", "SZR", "VBH")
ENTITIES_MC9 = ("abutment-wall", "chamfer-stop", "cusped-light",
                "extrados-ring", "label-stop", "soffit-panel")
COLORS_MC9 = ("umber", "celadon", "garnet", "zinc-white")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC9)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC9)
        ent = rng.choice(ENTITIES_MC9)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC9, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC9, n)]
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "schema": "anra-mixed-causal/v9-transfer",
