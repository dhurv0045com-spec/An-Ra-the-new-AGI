"""MIXED-CAUSAL-v4: fresh promotion fixture for self-model v4.

Generated ONLY after v4 freeze (commit 1861e0f). Fresh seed, entities,
prefixes. 180 tasks (30 per family × 6).
"""
...
SEED_MC4 = 20261102
PREFIXES_MC4 = ("CKP", "FZR", "JNW", "MYS", "TQL")
ENTITIES_MC4 = ("ashlar-block", "broach-spire", "cable-moulding",
                "dentil-course", "escarp-pin")
COLORS_MC4 = ("indigo", "amber", "jade", "coral")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC4)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC4)
        ent = rng.choice(ENTITIES_MC4)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC4, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC4, n)]
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "schema": "anra-mixed-causal/v5-promotion",
