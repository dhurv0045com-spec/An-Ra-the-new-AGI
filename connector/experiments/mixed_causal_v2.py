"""MIXED-CAUSAL-v2: fresh promotion fixture for self-model v3.

Generated ONLY after policy v3 was frozen (commit 3123625). Fresh seed,
entities, code prefixes. Same six families, same structure contract as v1.
"""
...
SEED_MC2 = 20261005
PREFIXES_MC2 = ("JRC", "MZV", "QXL", "TFB", "WKD")
ENTITIES_MC2 = ("abacus-frieze", "chamfer-stop", "drip-moulding",
                "empenage", "fillet-shoulder")
COLORS_MC2 = ("umber", "cerulean", "russet", "sage")
...
def _code(rng):
    return f"{rng.choice(PREFIXES_MC2)}-{rng.randrange(100, 1000)}"
...
def _make_task(family, task_id, rng):
    ...
    if family == "non_candidate":
        c = rng.choice(COLORS_MC2)
        ent = rng.choice(ENTITIES_MC2)
    ...
    if family == "composition":
        recs = [{"entity": e, "code": _code(rng)}
                for e in rng.sample(ENTITIES_MC2, 3)]
    ...
    recs = [{"entity": e, "code": _code(rng)}
            for e in rng.sample(ENTITIES_MC2, n)]
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "schema": "anra-mixed-causal/v3-promotion",
