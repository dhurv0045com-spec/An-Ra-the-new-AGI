...
FREEZE_SEED = 20260824
SCHEMA = "anra-ood3-battery/v1"

# Disjoint from OOD-1, OOD-2, and both SFT generators.
PREFIXES = ("QZM", "RVT", "SXL", "TBK", "UCN", "WDP", "XFJ", "YGB")
OBJECTS = ("bellfry", "causeway", "drumlin", "escarpment2", "flume", "grotto",
           "hayloft", "inlet", "jetty", "keystone", "lintel2", "moraine",
           "nightjar", "palisade", "quicksand", "rampart")
DISTRACTORS = ("The ferry runs twice a day.",
               "Spare parts are stored downstairs.",
               "The committee meets on Thursdays.",
               "A backup generator was installed.")
...
QUERIES = {
    "paragraph": "Return ONLY the code for {t}.",
    "table": "Code of {t}, nothing else:",
    "json": "Give the code value of {t}.",
    "dialogue": "The code for {t}, please — just the code.",
    "kv": "Value for {t}:",
    "mixed": "Code of {t}?",
}
...
def _p(line: str):
    if " holds code " in line:
        obj, code = line.split(" holds code ")
...
def build_items() -> list[dict]:
    ...
    for fmt in RENDERERS:
        for k in (2, 3, 4):
            plan.append((fmt, k))
    for fmt in ("paragraph", "table", "kv", "json", "dialogue", "mixed", "paragraph", "table"):
        plan.append((fmt, 5))
