...
cells.append({"cell_type": "code", "metadata": {}, "source": [
    ...
    "# refuse to run with visible CUDA) execute exactly as designed. The\n",
    "# suite runs in two fresh processes (bulk, then production-entry): one\n",
    "# 700-test process is OOM-killed (SIGKILL) on Colab-free RAM. Thread\n",
    "# cap 2 per preregistration and low memory.\n",
    "ENV = dict(os.environ, PYTHONPATH=\"/content/repo\",\n",
    "           CUDA_VISIBLE_DEVICES=\"\", OMP_NUM_THREADS=\"2\",\n",
    "           MKL_NUM_THREADS=\"2\", ANRA_TEST_DEVICE=\"cpu\")\n",
    "\n",
    "def run(*args):\n",
    ...
    "run(\"-m\", \"pytest\", \"tests/test_production_entry.py\", \"-q\")\n",
