...
def build() -> dict[str, object]:
    ...
    markdown = markdown.replace(
        "If either state reports `PARTIAL_SESSION`, save the Kaggle version/output and attach the complete saved `/kaggle/working/FORMATION_MUX_001` tree to the next run of this exact notebook. Do not attach only `FORMATION_MUX_001_RESULTS.zip`: that evidence bundle intentionally omits `resume.pt` and is not resumable. This notebook is for fresh starts only; for an existing Output, do not run its direct-launch Cell 2. Use `notebooks/CYMEK_FORMATION_MUX_001_RECOVERY_T4X2.ipynb`, which enforces the pinned same-kernel preflight, completed checkpoint/result immutability, and the automatic frozen sealed-finalization boundary.",
        "If either state reports `PARTIAL_SESSION`, save the Kaggle version/output and attach the complete saved `/kaggle/working/FORMATION_MUX_001` tree to the next run of this exact notebook. Do not attach only `FORMATION_MUX_001_RESULTS.zip`: that evidence bundle intentionally omits `resume.pt` and is not resumable. This notebook is for fresh starts only; for an existing Output, do not run its direct-launch Cell 2. Use `notebooks/CYMEK_FORMATION_MUX_001_RECOVERY_T4X2.ipynb`, which enforces the pinned same-kernel preflight, completed checkpoint/result immutability, and the automatic frozen sealed-finalization boundary. The result ZIP and campaign state stay under `/kaggle/working/FORMATION_MUX_001`; source code is cloned into `/kaggle/temp` so rerunning this notebook does not reset, clean, or remove user files in `/kaggle/working`.",
    )
