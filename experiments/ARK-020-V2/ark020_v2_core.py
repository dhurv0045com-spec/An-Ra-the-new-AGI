...
def deterministic_indices(seed: int, step: int, count: int, n: int, tag: str) -> list[int]:
    d = hashlib.sha256(f"ark020v2:{tag}:{seed}:{step}".encode()).digest()
...
def augmented_perm3(seed: int, step: int, idx: int) -> tuple[int, int, int]:
    ...
    d = hashlib.sha256(f"ark020v2-aug:{seed}:{step}:{idx}".encode()).digest()
...
def render_compose(t: Mapping[str, Any], chains, x_query: int, order) -> list[int]:
    """'Links: x gives m; …; m makes y; …; Trace: x to' -> y (two-hop, order-robust)."""
    ch = chains["chains"]
    ids = list(t["p"])
    for i in order:
        x, m, _y = ch[i]
...
def task_rows(cap: str, t: Mapping[str, Any], sem, idx, mode, seed: int, step: int):
    ...
    if cap in ("A", "B", "D"):
        ...
        return out
    # skill C: sem rows are (chains, queried_x, answer_y)
    for i in idx:
        ...
        if mode == "canonical":
            ...
            raise ValueError(mode)
        out.append((render_compose(t, chains, xq, order), int(a)))
    return out
