"""ARK-020 V2 core logic — torch-free, deterministic, testable.

Repairs relative to V1 core:
  - skill C replaced by two-hop relational composition (logically inferable;
    see TASK_VALIDITY_ANALYSIS.md);
  - per-phase order seeds are first-class constants and are bound into set identity;
  - phase-relative confirmation is the preregistered acquisition metric;
  - decide() implements the median phase-relative 1.5x rule exactly.

Controller functions still accept CONTROL metrics only; SEALED data never reaches them.
"""
...
# Per-phase order seeds: each phase streams with its OWN seed (V1 defect #3 fix).
...
# Capability lifecycle (blocker E): a capability is retention-eligible only after its
# acquisition gate confirms it. Acquisition metrics are NEVER retention failures.
LIFECYCLE_STATES = ("UNSEEN", "ACQUIRING", "ACQUIRED", "HEALTHY", "WARNING",
                    "DEGRADED", "FAILED", "RECOVERING", "RECOVERED", "DORMANT")


def lifecycle_state(cap_id: str, acquired_caps: set, phase: str | None,
                    latest_control, failure_since) -> str:
    if cap_id not in acquired_caps:
        return "ACQUIRING" if phase == cap_id else "UNSEEN"
    if failure_since is not None:
        return "DEGRADED"
    if latest_control is None:
        return "ACQUIRED"
    if healthy(latest_control):
        return "HEALTHY"
    if qualified(latest_control):
        return "WARNING"
    return "DEGRADED"


def retention_targets(acquired_caps: set, phase: str | None, phase_confirmed: bool) -> set:
    """Retention-eligible capabilities at this moment (mission #7/#8).

    During phase X, X is a retention target only AFTER its confirmation. A is
    acquired at parent time.
    """
    targets = set(acquired_caps)
    if phase is not None and phase_confirmed and phase in targets:
        pass  # confirmed -> already in acquired_caps
    if phase is not None and not phase_confirmed:
        targets.discard(phase)
    return targets


def record_retention_event(events: list, cap_id: str, step: int, phase: str,
                           eligible: bool, control_metrics) -> bool:
    """Record a retention failure ONLY for retention-eligible capabilities.

    Returns whether an event was recorded. Acquisition-phase metrics of the
    currently-learning skill are never forgetting events.
    """
    if not eligible:
        return False
    if qualified(control_metrics):
        return False
    events.append({"capability": cap_id, "failure_step": int(step), "phase": phase,
                   "was_acquired_before_phase": True, "control_metrics": dict(control_metrics)})
    return True
SKILL_FAMILY = {"A": "relational_binding", "B": "relational_binding",
                "C": "two_hop_composition", "D": "inverse_retrieval"}
PHASES = ("B", "C", "D")
PHASE_UPDATES = {"B": 2000, "C": 1500, "D": 1500}
PHASE_TASK_SLOTS = {"B": None, "C": 12, "D": 12}
...
GUARDIAN_SUCCESS_RULES = {
    ...
    "median_phase_relative_confirmation_ratio_max": 1.5,
...
def inverse_split5(owners: Iterable[int], objects: Iterable[int], seed: int) -> dict[str, list]:
    """Genuine inverse retrieval (V3 repair).

    Facts are presented FORWARD as (owner, object) pairs — rendered 'owner holds
    object'. The query is an OBJECT and the answer is its OWNER, so the model must
    invert the direction of the relation shown in context. The (object, owner)
    reversed pair is never presented or queried directly.
    """
    owners, objects = list(owners), list(objects)
    x = [tuple(zip(ot, bt)) for ot in itertools.combinations(owners, 3)
         for bt in itertools.permutations(objects, 3)]
    random.Random(seed).shuffle(x)
    if len(x) < 600:
...
def inverse_semantics(fs):
    """Rows (facts, queried_object, answer_owner) for the forward-presented pairs."""
    out = []
    for f in fs:
        for (owner, obj) in f:
            out.append((list(f), obj, owner))
    return out


def compose_chainsets(x_tokens: list[int], m_tokens: list[int], y_tokens: list[int],
                      seed: int) -> dict[str, list]:
    """Two-hop composition factsets.

    One chain set = 3 chains (x -> m -> y): 3 distinct X drawn from 6, the 3 M
    assigned bijectively, the 3 Y assigned bijectively. The Trace answer for a
    queried x is the y of its chain; both maps live in the prompt, so every
    answer is logically derivable (TASK_VALIDITY_ANALYSIS.md).
    """
    if not (len(x_tokens) == 6 and len(m_tokens) == 3 and len(y_tokens) == 3):
        raise RuntimeError("compose task needs 6 X + 3 M + 3 Y tokens")
    rng = random.Random(seed)
    sets: list[dict] = []
    for xs in itertools.combinations(sorted(x_tokens), 3):
        for ms in itertools.permutations(m_tokens):
            for ys in itertools.permutations(y_tokens):
                chains = tuple((x, m, y) for x, m, y in zip(xs, ms, ys))
                sets.append({"chains": chains})
    rng.shuffle(sets)
    if len(sets) < 600:
        raise RuntimeError(f"insufficient chain sets: {len(sets)}")
    return {"train": sets[:400], "parent_control": sets[400:450], "main_control": sets[450:500],
            "validation": sets[500:550], "sealed": sets[550:600]}


def compose_semantics(chainsets: Iterable[dict]) -> list[tuple[dict, int, int]]:
    """Semantics rows (chains, queried_x, answer_y); query x rotates over the 3 chains."""
    out = []
    for cs in chainsets:
        for (x, _m, y) in cs["chains"]:
            out.append((cs, x, y))
    return out
...
def build_all_tasks(token_ids: list[int]) -> dict[str, Any]:
    ...
    Cset = compose_chainsets(g["C"][0:6], g["C"][6:9], g["C"][9:12], 524220)
    D = inverse_split5(g["D"][:6], g["D"][6:12], 524221)
    return {"groups": g, "A": A, "B": B, "C": Cset, "D": D,
            "sem": {"A": {k: semantics(v) for k, v in A.items()},
                    "B": {k: semantics(v) for k, v in B.items()},
                    "D": {k: inverse_semantics(v) for k, v in D.items()},
                    "C": {k: compose_semantics(v) for k, v in Cset.items()}}}


def deterministic_indices(seed: int, step: int, count: int, n: int, tag: str) -> list[int]:
    d = hashlib.sha256(f"ark020v4:{tag}:{seed}:{step}".encode()).digest()
...
def augmented_perm3(seed: int, step: int, idx: int) -> tuple[int, int, int]:
    ...
    d = hashlib.sha256(f"ark020v3-aug:{seed}:{step}:{idx}".encode()).digest()
    return p[int.from_bytes(d[:8], "big") % 6]


def query_order_perm3(seed: int, step: int, idx: int) -> tuple[int, int, int]:
    """Chain order placing the queried chain last (order-robustness mode)."""
    q = idx % 3
...
def perm_from_hash(namespace: str, seed: int, step: int, idx: int):
    p = list(itertools.permutations(range(3)))
    d = hashlib.sha256(f"ark020v3-{namespace}:{seed}:{step}:{idx}".encode()).digest()
    return p[int.from_bytes(d[:8], "big") % 6]


def nonidentity_perm_from_hash(namespace: str, seed: int, step: int, idx: int):
    p = [x for x in itertools.permutations(range(3)) if x != (0, 1, 2)]
    d = hashlib.sha256(f"ark020v3-{namespace}:{seed}:{step}:{idx}".encode()).digest()
    return p[int.from_bytes(d[:8], "big") % 5]


def c_orders(mode: str, seed: int, step: int, idx: int):
    """Independently determined XM and MY block orders (positional-shortcut repair).

    The two blocks use DIFFERENT hash namespaces, so no evaluation mode can
    structurally guarantee that the queried X's ordinal position in XM matches its
    chain-Y's ordinal position in MY.
    """
    if mode == "canonical":
        return perm_from_hash("xm-can", seed, step, idx), perm_from_hash("my-can", seed, step, idx)
    if mode == "reversed":
        xm = perm_from_hash("xm-can", seed, step, idx)
        my = perm_from_hash("my-can", seed, step, idx)
        return tuple(reversed(xm)), tuple(reversed(my))  # still independent namespaces
    if mode == "query_order":
        # queried chain's XM relation moves last; MY block keeps its own order
        xm = perm_from_hash("xm-can", seed, step, idx)
        q = idx % 3
        rest = [i for i in xm if i != q]
        return tuple(rest) + (q,), perm_from_hash("my-can", seed, step, idx)
    if mode == "augmented":
        return perm_from_hash("xm-aug", seed, step, idx), perm_from_hash("my-aug", seed, step, idx)
    if mode == "nonidentity":
        return (nonidentity_perm_from_hash("xm-nid", seed, step, idx),
                nonidentity_perm_from_hash("my-nid", seed, step, idx))
    raise ValueError(mode)


def render_compose(t: Mapping[str, Any], chains, x_query: int, mode: str,
                   seed: int, step: int, idx: int) -> list[int]:
    """'Links: [x gives m;]* [m makes y;]* Trace: x to' -> y with independently
    permuted XM and MY blocks."""
    ch = chains["chains"]
    order_xm, order_my = c_orders(mode, seed, step, idx)
    ids = list(t["p"])
    for i in order_xm:
        x, m, _y = ch[i]
        ids += [int(x)] + list(t["g"]) + [int(m)] + list(t["s"])
    for i in order_my:
        _x, m, y = ch[i]
        ids += [int(m)] + list(t["k"]) + [int(y)] + list(t["s"])
    ids += list(t["q"]) + [int(x_query)] + list(t["e"])
    return ids[-MAX_PROMPT_IDS:]


def render_compose_tied(t: Mapping[str, Any], chains, x_query: int, order) -> list[int]:
    """MUTATION REFERENCE ONLY: the V2 renderer with mechanically tied orders.
    Exists so tests can prove the positional-shortcut detector catches it."""
    ch = chains["chains"]
    ids = list(t["p"])
    for i in order:
        x, m, _y = ch[i]
...
def task_rows(cap: str, t: Mapping[str, Any], sem, idx, mode, seed: int, step: int):
    out = []
    if cap in ("A", "B", "D"):
        for i in idx:
            f, q, a = sem[int(i)]
            if mode == "canonical":
                ...
                order = query_order_perm3(seed, step, int(i))
            elif mode == "augmented":
        ...
        return out
    # skill C: sem rows are (chains, queried_x, answer_y); XM/MY orders independent
    for i in idx:
        chains, xq, a = sem[int(i)]
        out.append((render_compose(t, chains, xq, mode, seed, step, int(i)), int(a)))
    return out


# ------------------------------------------------- adversarial C validity stats

def parse_rendered_compose(ids, t, xq):
    """Recover (m, y_true, ordinals, y_at_same_ordinal) from a rendered C prompt."""
    g, k = t["g"][0], t["k"][0]
    qpos = ids.index(t["q"][0])
    xm_pairs, my_pairs = [], []
    i = len(t["p"])
    while i < qpos - 1:
        if ids[i + 1] == g:
            xm_pairs.append((ids[i], ids[i + 2]))
            i += 4
        elif ids[i + 1] == k:
            my_pairs.append((ids[i], ids[i + 2]))
            i += 4
        else:
            i += 1
    m_of_x = next(m for (x, m) in xm_pairs if x == xq)
    xm_ord = [x for (x, _m) in xm_pairs].index(xq)
    my_ords = [m for (m, _y) in my_pairs]
    my_ord = my_ords.index(m_of_x)
    y_true = next(y for (m, y) in my_pairs if m == m_of_x)
    y_same_ordinal = my_pairs[xm_ord][1] if xm_ord < len(my_pairs) else None
    return {"m": m_of_x, "y_true": y_true, "xm_ordinal": xm_ord, "my_ordinal": my_ord,
            "y_at_same_ordinal": y_same_ordinal}


def positional_shortcut_score(sem, n: int, seed: int, mode: str = "canonical") -> float:
    """Fraction of sampled prompts where the SAME-ORDINAL Y equals the true answer.

    With independent XM/MY orders this must be near chance (1/3); under the tied
    V2 renderer it would be 1.0 (the mutation test proves the detector works).
    """
    import random as _r
    rng = _r.Random(seed)
    hits = 0
    ...
    for j in range(n):
        ...
        ids = render_compose(t, chains, xq, mode, seed, j, idx)
        info = parse_rendered_compose(ids, t, xq)
        if info["y_at_same_ordinal"] == info["y_true"]:
            hits += 1
    return hits / n


def m_matcher_oracle_accuracy(sem, n: int, seed: int) -> float:
    """Fraction of sampled prompts where X->M->Y recovery from the RENDERED prompt
    equals the true answer. Must be 1.0 — the intended operation is sufficient."""
    import random as _r
    rng = _r.Random(seed)
    hits = 0
    t = {"p": [901], "g": [902], "s": [903], "k": [904], "q": [905], "e": [906]}
    for j in range(n):
        idx = rng.randrange(len(sem))
        chains, xq, ans = sem[idx]
        ids = render_compose(t, chains, xq, "canonical", seed, j, idx)
        info = parse_rendered_compose(ids, t, xq)
        if info["y_true"] == ans:
            hits += 1
    return hits / n
...
def treatment(arm: str, ctrl: Mapping[str, Any], step: int, reg: dict,
    ...
    if arm == "STATIC_CAP16X":
        return [], cap16x
    states = ctrl["states"]
    # Blocker F repair: SPARSE64 must actually mean ~1/64 exposure (one slot every
    # two updates per capability); REPLAY32/EMERGENCY_CAP16X mean every update.
    eligible = []
    for c in old_caps:
        st = states.get(c, "PLASTIC")
        if st in ("REPLAY32", "EMERGENCY_CAP16X"):
            eligible.append(c)
        elif st == "SPARSE64":
            if step % 2 == 0:
                eligible.append(c)
    ordered = risk_order(reg, eligible)
...
def decide(arm_results: Mapping[str, list[Mapping[str, Any]]]) -> dict[str, Any]:
    """Frozen verdict logic. Acquisition comparisons use PHASE-RELATIVE confirmation."""
    ...
    key = lambda r: (int(r["parent_seed"]), int(r["b_order_seed"]))  # noqa: E731
    ...
    for skill in ("B", "C", "D"):
        ok = sum(1 for r in maps["PLASTIC_HIGH"].values()
                 if r["phase_confirmation"].get(skill) is not None)
        if ok < 3:
            return {"verdict": f"INCONCLUSIVE_FORMATION_INSTABILITY_PHASE_{skill}",
                    "authorized": False, "plastic_qualifications": ok, "phase": skill}

    ph = [maps["PLASTIC_HIGH"][k] for k in keys]
    ph_fail_sets = sum(1 for r in ph if r.get("retention_failure_events"))
    ...
    # Phase-relative medians of the plastic reference (defect #4 fix: no global offsets).
    ...
    for arm in GUARDIAN_ARMS:
        ...
        # Preregistered rule: median-vs-median per phase (NOT per-set, NOT global steps).
        all_confirmed = all(r["phase_confirmation"].get(p_) is not None
                            for r in rows for p_ in PHASES)
        conf_ok = bool(all_confirmed) and all(
            median([r["phase_confirmation"][p_] for r in rows]) <=
            GUARDIAN_SUCCESS_RULES["median_phase_relative_confirmation_ratio_max"] * ph_med[p_]
            for p_ in PHASES)
        gaps, nll_rel, duties, replay_fracs = [], [], [], []
        ...
        prevention = sum(1 for r in rows if not r.get("retention_failure_events"))
        ...
        details[arm] = {
            ...
            "phase_relative_confirmation_within_1_5x_plastic_median": conf_ok,
            "plastic_phase_relative_medians": ph_med,
    ...
    reactive_ok = details["GUARDIAN_REACTIVE"]["qualifies"]
