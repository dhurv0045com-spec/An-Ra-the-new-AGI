...
executable identity receipt for the real hardened operator path.
"""
...
def _advance_impl(R, st, tasks, tt, bufs, dose_b, start: int, stop: int) -> list:
    rows = []
    names = R.pnames(st["m"])
    try:
...
def install_advance_only(R) -> None:
    """Patch only the device-consistent helper, without changing identity hooks.

    Used by the inherited test bootstrap so test import order cannot alter the
    hardened operator's executable-identity composition.
    """
    if getattr(R, "_ARK020_V4_A12_ADVANCE_INSTALLED", False):
        return

    def _advance(st, tasks, tt, bufs, dose_b, start: int, stop: int) -> list:
        return _advance_impl(R, st, tasks, tt, bufs, dose_b, start, stop)

    R._advance = _advance
    R._ARK020_V4_A12_ADVANCE_INSTALLED = True
...
def install(R) -> None:
    """Install the full A1.2 operator repair exactly once."""
    if getattr(R, "_ARK020_V4_A12_DEVICE_GUARD_INSTALLED", False):
        return
    install_advance_only(R)
