...
OUT = Path("/content/drive/MyDrive/genisis-arkenstone/ARK020_V3_CONTINUAL")
...
def real_batch(buf, n, seed, step, d, tag="main"):
    ...
    for slot in range(n):
        z = int.from_bytes(hashlib.sha256(
            f"ark020v3-real:{tag}:{seed}:{step}:{slot}".encode()).digest()[:8], "big") % (len(buf) - need + 1)
...
def run_set_arm(ps, bs, arm, dose_b, parent_state, cap16, bufs, tt, tasks, d, deadline,
    ...
    expected = {"schema": "arkenstone-ark020-v3-arm-ckpt/v1", "parent_seed": ps,
    ...
    def timebox(phase_idx, phase, step):
        ...
        savej(pp, {"schema": "arkenstone-ark020-v3-partial/v1", "status": "PARTIAL_SESSION",
    ...
    for phase_idx in range(start_phase, len(C.PHASES)):
        ...
        for step in range(start, C.PHASE_UPDATES[phase] + 1):
            ...
            if step % C.CONTROL_EVERY == 0:
                ...
                session_log(f"ARK020V3 [{ps}/{bs}/{arm}/{phase}] {step}/{C.PHASE_UPDATES[phase]} "
            ...
            if step % C.MEASURE_EVERY == 0:
                ...
                measure_trace.append({"step": gstep, "phase": phase, "sealed": sealed,
                                      ...
                                      "diagnostics": diag})
                savej(pp, {"schema": "arkenstone-ark020-v3-partial/v1", "status": "RUNNING",
                           "parent_seed": ps, "b_order_seed": bs, "arm": arm, "phase": phase,
    ...
    r = {"schema": "arkenstone-ark020-v3-arm/v1", "status": "COMPLETE", "parent_seed": ps,
         "b_order_seed": bs, "c_order_seed": c_seed, "d_order_seed": d_seed,
...
def _fresh_state(parent_state, seeds: dict) -> dict:
    m, o, sc = restore(parent_state, d := torch.device("cpu") if not torch.cuda.is_available()
...
def exact_resume_smoke(parent_state, dose_b, bufs, tt, tasks, d) -> dict:
    ...
    a = _fresh_state(parent_state, seeds)
    t1 = _advance(a, tasks, tt, bufs, dose_b, 1, 10)
    h1 = state_hashes(a)
    del a
    b = _fresh_state(parent_state, seeds)
    t2a = _advance(b, tasks, tt, bufs, dose_b, 1, 5)
    parent_sha = V3.state_hash(parent_state["model"])
    task_hash = hjson({k: hjson(tasks[k]) for k in ("A", "B", "C", "D")})
    expected = {"schema": "arkenstone-ark020-v3-arm-ckpt/v1", "parent_seed": 31801,
    ...
    r = {"schema": "arkenstone-ark020-v3-exact-resume/v1", "status": "PASS" if ok else "FAIL",
...
def package(root: Path, final=False):
    ...
    savej(root / "ZIP_MANIFEST.json", {"schema": "arkenstone-ark020-v3-manifest/v1", "members": manifests})
    name = "ARKENSTONE_ARK020_V3_CONTINUAL_RESULTS.zip" if final else "ARKENSTONE_ARK020_V3_CONTINUAL_PARTIAL.zip"
    z = root / name
...
def resume_scan(drive_ok: bool = True) -> dict:
    ...
    try:
        ...
        return info
    checks: dict[str, str] = {}
    if x.get("schema") != "arkenstone-ark020-v3-arm-ckpt/v1":
        checks["schema"] = "FAIL"
    checks["experiment_version"] = "PASS" if str(x.get("schema", "")).startswith(
        "arkenstone-ark020-v3") else "FAIL"
    ...
    if receipt_data:
        ...
        src = {s_["seed"]: s_["model_sha256"] for s_ in receipt_data.get("sources", [])}
        if x.get("parent_seed") in src:
            checks["parent_hash_vs_receipt"] = ("PASS" if x.get("parent_sha") == src[x["parent_seed"]]
                                                else "FAIL")
        else:
            checks["parent_hash_vs_receipt"] = "PARTIAL (source not in receipt)"
    else:
        checks["entry_receipt"] = "PARTIAL_IDENTITY_CHECK (receipt absent)"
    dose_file = OUT / "DOSE_SELECTION.json"
    if x.get("dose_b") is not None and dose_file.exists():
        try:
            dd = json.loads(dose_file.read_text())
            checks["dose_vs_record"] = ("PASS" if int(dd.get("selected_b_slots", -1)) == int(x["dose_b"])
                                        else "FAIL")
        except Exception:
            checks["dose_vs_record"] = "PARTIAL_IDENTITY_CHECK"
    else:
        checks["dose_vs_record"] = "PARTIAL_IDENTITY_CHECK (no dose record)"
...
def calibrate(parent_state, dose_b, bufs, tt, tasks, d):
    ...
    r = {"schema": "arkenstone-ark020-v3-runtime/v1", "status": "PASS", "per_update_seconds": rec,
...
def cap_calibration(ps, bs, parent, bufs, tt, tasks, d, deadline):
    ...
    r = {"schema": "arkenstone-ark020-v3-capcal/v1", "parent_seed": ps, "b_order_seed": bs,
...
def cpu_integration_smoke(root: Path) -> dict:
    ...
    report: dict[str, Any] = {"schema": "arkenstone-ark020-v3-cpu-smoke/v1", "steps": []}
...
def run_all():
    ...
    try:
        ...
        savej(OUT / "ENTRY_RECEIPT.json", {
            "schema": "arkenstone-ark020-v3-entry/v1",
        ...
        for seed in C.PARENT_SEEDS:
            ...
            if r.get("status") != "QUALIFIED":
                result = {"schema": "arkenstone-ark020-v3-result/v1", "status": "BLOCKED_BEFORE_MAIN",
                          "verdict": "INCONCLUSIVE_PARENT_GATE_FAILED", "seed": seed, "authorized": False}
                savej(OUT / "ARK-020_V3_RESULT.json", result)
        ...
        savej(OUT / "PREEXECUTION_GATE.json", {
            "schema": "arkenstone-ark020-v3-preexec/v1", "status": "PASS",
        ...
        result = {"schema": "arkenstone-ark020-v3-result/v1", "status": "COMPLETE",
        ...
        state = {"schema": "arkenstone-ark020-v3-session/v1", "status": "PARTIAL_SESSION",
...
def main():
    ...
    try:
        ...
        OUT.mkdir(parents=True, exist_ok=True)
        savej(OUT / "ARK-020_V3_FAILURE.json", {"schema": "arkenstone-ark020-v3-failure/v1",
