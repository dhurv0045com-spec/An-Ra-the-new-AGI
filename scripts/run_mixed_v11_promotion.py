...
def main() -> None:
    import connector.experiments.mixed_causal_v11 as mc
    ...
    FIX = mc.fixture_hash()
    pol = json.loads((ROOT / "output/self_model_v9pol.json").read_text(encoding="utf-8"))
    lam = pol["lambda"]
    ...
    for ti, task in enumerate(tasks):
        ...
        if task["output_arity"] == 2 and len(candidates) >= 2:
            applicable.extend(["EXACT_PAIR", "SLOT_PAIR"])
        ...
        feats_raw = feats_raw + [feats_raw[4] - feats_raw[3]]
        ...
        utilities = {a: predict(a) - lam * mc.COSTS.get(a, 3)
                     for a in applicable}
        adaptive = min(utilities,
                       key=lambda a: (-utilities[a],
                                      mc.COSTS.get(a, 3)))
        ...
        for action in applicable:
            if action == "NO_CHANGE":
                ...
                emitted, _ = constrained_greedy(model, tok, base_prompt,
                                                candidates)
            elif action in ("EXACT_PAIR", "SLOT_PAIR"):
                if action == "EXACT_PAIR":
                    if norm_scores is not None:
                        ranked = norm_scores
                    else:
                        ranked = [completion_logprob(model, tok,
                                                     base_prompt, f" {c}.")
                                  for c in candidates]
                    order2 = sorted(range(len(ranked)),
                                    key=lambda i: ranked[i], reverse=True)
                    emitted = (f" {candidates[order2[0]]}"
                               f" {candidates[order2[1]]}.")
                else:
                    from connector.experiments.slot_pair import (
                        slot_pair_scores, choose_pair, pair_emission)
                    def _score(prompt, completion):
                        return completion_logprob(model, tok, prompt,
                                                  completion)
                    s0, s1 = slot_pair_scores(_score, model,
                                              tok, base_prompt, candidates)
                    f_code, s_code = choose_pair(candidates, s0, s1)
                    emitted = pair_emission(f_code, s_code)
            else:
                emitted = f" {norm_pick_code}."
            results[action] = {
                "output": (emitted or "").strip()[:64],
                "pass": bool(mc.verify(task, emitted)),
                "cost": mc.COSTS.get(action, 3),
                # full retained outputs for VIE qualification
                "retained_output": (emitted or "").strip(),
            }
    ...
    policies["ADAPTIVE_v9pol"] = run_policy(lambda r: r["adaptive_action"])
    ...
    (ROOT / "output/mixed_causal_v11_promotion.json").write_text(
