...
def main() -> None:
    ...
    FIX = mc.fixture_hash()
    pol = json.loads((ROOT / "output/self_model_v7.json").read_text(encoding="utf-8"))
    lam = pol["lambda"]
    ...
    _ap.add_argument("--checkpoint",
                     default="checkpoints/anra-v4-20k-sft6-queryswap-replication.pt")
    _args = _ap.parse_args()
    CKPT_PATH = _args.checkpoint
    model, _, ident = load_core_checkpoint(CKPT_PATH, legacy_unverified=True)
    ...
    for ti, task in enumerate(tasks):
        ...
        adaptive = min(utilities,
                       key=lambda a: (-utilities[a], mc.COSTS[a]))
    ...
    policies["ADAPTIVE_v7"] = run_policy(lambda r: r["adaptive_action"])
    ...
    report = {
        ...
        "policy_frozen_commit": "f3b55cf (unchanged; replication draw)",
        "n_tasks": n,
        "policies": policies,
    ...
    (ROOT / "output/mixed_causal_v9_transfer.json").write_text(
