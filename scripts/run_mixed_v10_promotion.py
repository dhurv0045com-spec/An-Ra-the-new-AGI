...
def main() -> None:
    ...
    FIX = mc.fixture_hash()
    pol = json.loads((ROOT / "output/self_model_v8pol.json").read_text(encoding="utf-8"))
    lam = pol["lambda"]
        ...
        for action in applicable:
            if action == "NO_CHANGE":
                ...
                emitted, _ = constrained_greedy(model, tok, base_prompt,
                                                candidates)
            elif action == "EXACT_PAIR":
                if norm_scores is not None:
                    ranked = norm_scores
                else:
                    ranked = [completion_logprob(model, tok, base_prompt,
                                                 f" {c}.")
                              for c in candidates]
    ...
    policies["ADAPTIVE_v8pol"] = run_policy(lambda r: r["adaptive_action"])
    ...
    (ROOT / "output/mixed_causal_v10_promotion.json").write_text(
