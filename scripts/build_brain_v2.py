...
def _resolve_checkpoint_path(checkpoint_path: str) -> Path:
    ...
    if os.name == "nt" and raw.startswith("/tmp/"):
        ...
        print(f"[build_brain_v2] remapping temporary checkpoint path to {local_tmp}", flush=True)
        return local_tmp
...
def _prepare_resume_target(checkpoint_path: Path, resume_from: str | None) -> None:
    ...
    if candidate is not None and candidate.exists():
        ...
        print(f"[build_brain_v2] restored checkpoint: {candidate} -> {checkpoint_path}", flush=True)
...
def train_anra_v2(
    *,
    ...
    print(f"  Resuming     : step {global_step:,}  |  best loss {best_loss if math.isfinite(best_loss) else float('inf'):.4f}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_path", default=str(canonical_v2_checkpoint("brain").name))
