...
class WorkflowExecutor:
    def __init__(
        ...
        *,
        ...
        self.cbf_check = cbf_check or (lambda _goal: True)
        self.transition_path = Path(transition_path) if transition_path else None

    def execute(
        self, workflow: Workflow, current_state: dict[str, object] | None = None
    ) -> Workflow:
        ...
        for index in range(workflow.current_index, len(workflow.skills)):
            ...
            next_state = {
                **state,
                ...
                "success": result.success,
                "observations": [
                    observation.natural_language_summary for observation in result.observations
                ],
            }
