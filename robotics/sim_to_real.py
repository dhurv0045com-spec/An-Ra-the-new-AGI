...
def decide_next_mode(
    *,
    ...
    if current_mode == "simulation":
        ...
        return SimToRealDecision(
            "shadow",
            passed,
            "simulation gate" if passed else "need >=80% randomized simulation success",
        )
    if current_mode == "shadow":
        passed = shadow_anomaly_rate <= 0.05
        return SimToRealDecision(
            "supervised", passed, "shadow gate" if passed else "shadow anomaly rate too high"
        )
    if current_mode == "supervised":
        passed = supervised_success >= 0.90
        return SimToRealDecision(
            "promoted", passed, "supervised gate" if passed else "need >=90% supervised success"
        )
