...
class CIVWatcher:
    ...
    def _classify(self, score: float) -> int:
        if score >= self.LEVEL_1_FLOOR:
            return 0
        if score >= self.LEVEL_2_FLOOR:
            return 1
        if score >= self.LEVEL_3_FLOOR:
            return 2
        return 3
    ...
    def _respond(self, score, delta, velocity, level) -> dict:
        base = dict(
            ...
            halt_training=False,
            sovereignty_hold=False,
            identity_ratio_boost=0.0,
        )
        if level == 0:
            return {**base, "action": "none", "message": f"CIV stable at {score:.4f}."}
        if level == 1:
            return {
                **base,
                "identity_ratio_boost": 0.10,
