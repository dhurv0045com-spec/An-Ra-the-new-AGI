...
def chance_report(k: int, n: int, chance: float | None) -> dict:
    lo, hi = wilson(k, n)
...
def classify_capability(n: int, raw_k: int, oracle_rate: float | None,
    ...
    return {"capability": cap, "raw_wilson95": [round(raw_lo, 4), round(raw_hi, 4)],
            ...
            "chance_report": chance_report(raw_k, n, chance), "notes": notes}
...
def assess_identifiability(n: int, n_failures: int, n_discordant: int,
    ...
    flags = {
        ...
        "oracle_near_chance": (oracle_rate is not None and chance is not None
                               and oracle_rate <= chance + 0.05),
