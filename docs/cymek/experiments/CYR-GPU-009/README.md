...
Historical one-shot Colab experiment. It replaces V8 after the operator's real hardware calibration proved the V8 minimum campaign was too broad for the assigned free GPU before any science ran.

This notebook is a historical launcher from a superseded campaign and has
been removed from the active `notebooks/` list. Do not start a new CYR-GPU-009
run; use the current research roadmap.
