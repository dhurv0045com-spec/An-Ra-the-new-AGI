...
|---|---|---|---|
| G1 | **CYR-GPU-014-R1C launcher pins `6653b4ce3a7e61b8fdffd3236c62c67622084dec`, which is an ancestor of NO live branch and is NOT served by GitHub** (API: "No commit found for SHA"; verified 2026-09-13). The commit is a history-rewrite orphan whose code content is byte-identical inside reachable `9adbf68`/`f2c27a6`, and the RUN_READINESS_V4 blob `a62d18d` is identical on `f2c27a6`. | Notebook v4 does `git clone --depth 300` + `git checkout 6653b4ce` with `check=True` → fresh-clone operator launch WILL fail. `R1C_READY_FOR_OPERATOR_CUDA_RUN` is currently not executable as documented. | Rebind launcher to a pushed commit after a frozen-identity review; alternatively push `6653b4ce` to a ref. Requires an explicit amendment because it touches a frozen executable identity — deliberately NOT done by this audit. |
...
| G11 | R1C pre-rewrite notebook revisions (`8162c22` @ `e444249`, `8eb3b09` @ `8adedab`) are unreachable; live `f2c27a6` carries `bb9789b`. | Historical binder chain for the R1C launcher is not ref-anchored (compounds G1). |
...
| G19 | Citadel 500M production-path audit is pinned to `28bf57a` (2026-09-06); cymek-500m-readiness has advanced ~236 commits since. The MISSING/AMBIGUOUS classifications are stale in the direction of optimism OR pessimism — an updated audit is needed, not assumed. |
