...
| Post-G90 decay is STOCHASTIC (data-order-dependent): Colab fork with different data order showed NO decay, while local independent run showed collapse on the same seed | ARK-002B/005 Colab vs local 4050 cross-device comparison | the decay is triggered by unlucky batch sequences, not by an inevitable optimization drift; LR-threshold prevention is therefore more valuable (eliminates the risk rather than reducing its probability) |
| High-LR decay is NOT universal: seed 707 stable at full LR, seed 808 transient dip + recovery | ARK-007 | the LR-threshold law is a risk factor, not a deterministic cause; some seeds naturally consolidate |
