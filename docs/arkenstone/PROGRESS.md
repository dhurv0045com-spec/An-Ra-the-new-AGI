...
| 2026-09-12 | arkenstone-agent | **ARK-020 preregistered + implemented (NOT executed)** | Multi-skill continual cognition generalization: 4 structurally distinct skills (binding ×2, successor-rule induction with never-trained sealed keys, inverse retrieval), 7 arms (reactive/predictive/hybrid Guardian vs static replay/cap), capability registry, risk-allocated replay with efficiency gate; preregistration frozen at `e2e6e09` BEFORE implementation; 29/29 pure tests green; mission-brief V4 believed-result audited as NOT in repository and excluded as design input |
| 2026-09-12 | arkenstone-agent | **Two-controller synthesis + ARK-020 novelty audit** | Formation (Cymek R1/R1B/R1C) vs preservation (Arkenstone laws) decomposition documented; novelty honestly classified — sparse replay/replay-permits-movement = extension/new-empirical at micro scale, allocation+de-escalation = combination; protection-cost slope and predictive-prevention flagged as candidate new discoveries pending ARK-020 outcomes |
| 2026-09-12 | arkenstone-agent | **ARK-020 V2 preregistered + implemented + 36 tests green (NOT executed)** | Scientific repair: 5 V1 blocking defects confirmed with line evidence and fixed (V4-call shapes, impossible successor-cycle C → two-hop composition, unused phase seeds, global-vs-phase confirmation bias, resume overclaim); V4 result recorded as externally-audited evidence; real integration-contract tests + CPU smoke added after numpy/torch installed locally |
| 2026-09-12 | arkenstone-agent | **ARK-020 V4 preregistered + implemented + 39 tests green (NOT executed)** | Continuation audit confirmed all 7 claimed V3 blockers live (untested resume smoke with tuple mismatch; C positional shortcut; D pre-reversed; scan-before-mount + identity overclaim; CLI contract broken; lifecycle accounting counted acquisition as forgetting; SPARSE64 mislabeled) plus vacuous construct. V4 repairs all: lifecycle retention events, cadence-true replay names (320/640 per 640 updates), acquired-parent identity receipts, local dose import, zero-write historical reuse, forced timebox. 134/134 lineage tests green |
| 2026-09-12 | arkenstone-agent | **ARK-020 V3 preregistered + implemented + 27 tests green (NOT executed)** | Continuation audit confirmed all 4 claimed V2 blockers live (production exact-resume smoke could not execute and was never tested; C positional shortcut 1.0000 under tied orders; D pre-reversed so not inverse; scan before Drive mount with identity overclaim) + vacuous test construct. All repaired: independent XM/MY hash namespaces (shortcut 0.3377 ~ chance, oracle 1.0), true-inverse D, executed production smoke with 12 hashed state classes, truthful resume scan with 6 SAFE ACTIONs, structured lock. V1/V2 immutable. Executable frozen at 13a50c2 with blob SHAs in RUN_READINESS |
