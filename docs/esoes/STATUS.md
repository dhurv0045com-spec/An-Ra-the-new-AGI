...
Verification: **120 CPU tests pass** (the P35 checkpoint, durability, runner, distributed-shard, and target-preflight contract tests are framework-independent; the recorded P35 canary itself ran on the RTX 4050). The last pushed exact-head GitHub Actions run is green; CI also regenerates the deterministic scoring, local CPU/CUDA parity, implementation, training-transaction, and local durability receipts byte-for-byte. The repaired E0 receipt is schema 2 / generator 0.4.0: pooled bag-of-words fell from the reproduced false-green 81.77% to 4.17%, and lexical overlap from 71.09% to 10.16%, versus a 10.37% casewise null. Exact random-weight middle-P35 scoring over real 16k/24k/32k tokenizers has zero CPU/CUDA prediction mismatches across 486 scores, but rejects all naive modes because length/tokenization bias remains. The local framework-neutral transaction, exact middle-P35 join, fail-closed runner lifecycle, and distributed rank-shard schema pass content-addressed publication, clean-copy restore, exact token accounting, model/optimizer/scheduler/RNG/cursor/ledger binding, semantic cursor/ledger checks, complete-rank/collective reconciliation, failure recovery, and continuation equivalence. The local immutable CAS canary additionally passes upload/redownload equality and corruption rejection. The target preflight reports `BLOCKED_TORCH_XLA` with explicit missing dependencies on this CPU host rather than pretending target readiness. Evaluation, durability, and promotion now have separate fail-closed, hash-bound contracts. Tokenizer, E1/E2/E3 planning, full-stack, initialization/QK/precision/RoPE/real-update/cursor receipts and package boundaries also pass. The RTX 4050 path remains operational. No shape, learned-quality, production scoring, remote durability, target-TPU, or long-run precision claim is promoted without the relevant evidence.

Next action: **resolve scoring policy without outcome leakage, then validate the checkpoint join on the target topology.** Preregister a bias-resistant calibration against fresh null surfaces and reproduce it on target TPU; do not choose it from trained-model outcomes. The local exact-P35 join is complete; next is distributed rank/RNG/shard integration, remote immutable upload→redownload→clean restore, and TPU/XLA parity. External natural/T2 custody and a representative tokenizer corpus remain required for E0/E1 promotion. Do not start the 250M main run.
