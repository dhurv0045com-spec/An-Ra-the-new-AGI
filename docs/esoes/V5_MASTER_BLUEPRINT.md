...
The scorer boundary is separately executable in `artifacts/e0/scoring_adapter_certificate.json`. It enforces suffix-only candidate traces, hash-bound adapter identity, sum/token/byte aggregation, complete candidate rotation, and oracle/broken/random controls. The deterministic null exposed that summed likelihood selects the fewest-token candidate 65.625% of the time and byte normalization selects the constructed first-token/token-density pattern 84.375%; token normalization is less biased but is not promoted from fake logits. Status is **CONTRACT PASS / DEVICE PENDING**: no production scoring mode exists until exact random-weight P35 × real 16k/24k/32k tokenizers passes length/tokenization/position and CPU/CUDA parity gates.
