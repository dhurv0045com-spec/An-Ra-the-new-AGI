...
Status: the framework-neutral transaction canary binds model/optimizer/scheduler/RNG/cursor/ledger/state inventories, enforces a single-writer parent fence, verifies every component hash/size, survives crashes before/after publication, rejects missing/corrupt components, and proves uninterrupted versus clean-copy resume over a deliberately non-divisible 10-token budget (`4+4+2`). Separately, the exact-P35 canary proves real model/optimizer update-resume and the cursor canary proves packed-stream continuation. **EVIDENCE-BACKED local components and transaction semantics; NOT YET an atomic real-P35, distributed, remote-durable, or TPU checkpoint.** Cadence remains **STRONG INFERENCE** pending target serialization/storage measurement.
