...
`artifacts/e2/scoring_policy_preregistration.json` freezes that next experiment before any result exists: 256 independent triplets, five development and five fresh P35 seeds, crossed surface/hidden roles, two four-context neutral panels, exact token-length/suffix matching, DC-PMI primary, contextual calibration secondary, and naive modes retained only as falsification controls. An audit caught and invalidated fixture v1 before powered execution because hidden role was perfectly determined by surface family; fixture v2 now gates every 6×3 contingency cell. Equivalence uses seed-clustered Student-t TOST with Holm correction over the full 132-hypothesis eligible family. Decoy, exact full-rank neutral-panel, intervention, CPU/CUDA, and later TPU gates are conjunctive. The resumable shard runner has passed a discarded one-group CUDA execution smoke; no powered development result exists. The consumed six-group pilot cannot confirm the policy, and trained-model outcomes cannot alter it. **PREREGISTERED / EXECUTABLE / POWERED RUN NOT YET EXECUTED.**
