...
- `32 passed` (operational O01–O10 acceptance, single invocation).
- `53 passed, 5 skipped` (checkpoint + learning + contracts).
- Combined: `120 passed, 5 skipped` across all eight files.
...
| O05 matched E2 | `phases/e2.py` (rewritten evaluation core: matched live groups, B + A restores, 7 modes with per-mode adapters/checkpoints, goal-swap before render, contradiction probes, complementary coverage, planner calibration gap, origin-gated evidence) | `O05MatchedEvaluationTests` (2): doubles complete fixture with 7 mode stats, paired deltas, goal-swap/contradiction/complementary rows, `k8-live-eval/v1` source; A-only parent fails. Production smoke (random-init B/A payloads, ProductionOps, 1 mechanism): completed, learned-campaign via real parents plus model-origin generations (parse-failure origins propagate; double world predictions stay non-model), 0 optimizer updates, controls measured, symbolic ceiling succeeds. `verify_local` failure paths preserved. | Owner run: 32 matched mechanisms × 7 modes on frozen B/A payloads; paired B−A deltas, goal-swap table, contradiction detection rate, complementary coverage, planner predicted-vs-actual gap. |
